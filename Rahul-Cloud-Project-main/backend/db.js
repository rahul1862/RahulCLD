...
if (isTest) {
  ...
  const bootstrap = new Client({ connectionString: baseConnectionString, ssl });
  ...
  pool = new Pool({ connectionString: testUrl.toString(), ssl });
} else {
  pool = new Pool({ connectionString: baseConnectionString, ssl });
}
...
const SEED = [
  ...
  { name: "Rahul Dwyer",      email: "RahulPohwani@greenfieldanalytics.com", address: "88 castle St, Dublin",      phone: "+353 83 101 9990", daysAgo: 5 },
];

const { rows: [{ c: userCount }] } = await query("SELECT COUNT(*) AS c FROM users");
...
// rough numbers, just enough for the finance page to have something to show on first load
const TX_SEED = [
  { type: "income",  category: "Subscriptions", description: "Marenfoods annual plan",           amount: 2400, daysAgo: 2 },
  { type: "income",  category: "Subscriptions", description: "Brightside Capital annual plan",    amount: 1800, daysAgo: 6 },
  { type: "income",  category: "Setup fees",    description: "Kilbride & Co onboarding",          amount: 350,  daysAgo: 9 },
  { type: "expense", category: "Hosting",       description: "Render API + static site",          amount: 25,   daysAgo: 1 },
  { type: "expense", category: "Software",      description: "GitHub Actions minutes",            amount: 12,   daysAgo: 4 },
  { type: "income",  category: "Subscriptions", description: "Northgate Logistics annual plan",   amount: 2100, daysAgo: 15 },
  { type: "expense", category: "Support",       description: "Contractor support hours",          amount: 480,  daysAgo: 18 },
  { type: "income",  category: "Setup fees",    description: "Verity Health onboarding",          amount: 350,  daysAgo: 22 },
  { type: "expense", category: "Hosting",       description: "Render API + static site",          amount: 25,   daysAgo: 32 },
  { type: "income",  category: "Subscriptions", description: "Lakeshore Studio annual plan",      amount: 1500, daysAgo: 35 },
  { type: "expense", category: "Software",      description: "Domain renewal",                    amount: 14,   daysAgo: 40 },
  { type: "income",  category: "Subscriptions", description: "Ashford Partners annual plan",      amount: 1200, daysAgo: 48 },
  { type: "expense", category: "Hosting",       description: "Render API + static site",           amount: 25,   daysAgo: 62 },
  { type: "income",  category: "Setup fees",    description: "Greenfield Analytics onboarding",   amount: 350,  daysAgo: 70 },
  { type: "expense", category: "Support",       description: "Contractor support hours",          amount: 480,  daysAgo: 75 },
];

const { rows: [{ c: txCount }] } = await query("SELECT COUNT(*) AS c FROM transactions");
