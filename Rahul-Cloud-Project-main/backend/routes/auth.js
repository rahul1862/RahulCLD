...
// Mounts the authentication endpoints on the outer app. These are registered
// BEFORE the blanket `requireAuth` guard in appFactory, so `/api/auth/login`
// and `/api/auth/register` stay public while every other `/api/*` route
...
export async function setupAuth(app, { db }) {
  ...
  app.post("/api/auth/register", registerLimiter, async (req, res) => {
    ...
    if (result.errors) {
      ...
      return res.status(status).json({ message: Object.values(result.errors)[0], errors: result.errors });
    }
