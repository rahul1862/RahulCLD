// Post-deploy smoke test against the live Render URLs.
//
//   npm run smoke:deployed
//   API_URL=https://my-api.onrender.com WEB_URL=https://my-web.onrender.com npm run smoke:deployed
//
// Verifies the deployed API and static site respond correctly end-to-end:
// health probe, public root, JWT login, an authenticated read, and that
// protected routes reject anonymous access. Exits non-zero if any check fails.
...
const req = (url, opts) => fetch(url, { signal: AbortSignal.timeout(TIMEOUT), ...opts });

console.log(`\nSmoke testing deployed app`);
console.log(`  API: ${API_URL}`);
console.log(`  Web: ${WEB_URL}\n`);

// 1. Static site serves the SPA shell.
try {
...
try {
  ...
  record("API /health is healthy", res.status === 200 && body.status === "healthy", `HTTP ${res.status}`);
} catch (err) {
...
// 5. Login issues a token, and it unlocks a protected read.
let token = null;
try {
  ...
  const body = await res.json().catch(() => ({}));
  token = body.token;
  record("login returns a JWT for demo credentials", res.status === 200 && !!token, `HTTP ${res.status}`);
} catch (err) {
...
if (token) {
  try {
    const res = await req(`${API_URL}/api/users`, { headers: { Authorization: `Bearer ${token}` } });
    const body = await res.json().catch(() => null);
    record("authenticated GET /api/users returns a list", res.status === 200 && Array.isArray(body), `HTTP ${res.status}`);
  } catch (err) {
