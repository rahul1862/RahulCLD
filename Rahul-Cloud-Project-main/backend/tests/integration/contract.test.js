...
//
// Boots the real API and asserts the exact JSON shapes the React client
// consumes (frontend/src/services/api.js + hooks). If the API ever drifts
// from what the frontend expects, this fails here instead of in the browser.
...
test("a created user echoes the _id + fields the frontend reads", async () => {
  ...
  // The client keys off `_id` (Mongo-style) and renders these fields.
  assert.ok(hasKeys(res.body, ["_id", "name", "email", "company", "address", "phone", "createdAt"]));
});
...
test("GET /api/pnl returns the finance summary shape", async () => {
  ...
  assert.equal(res.status, 200);
  assert.ok(hasKeys(res.body, ["totalIncome", "totalExpense", "netProfit", "byCategory", "byMonth"]));
  assert.ok(Array.isArray(res.body.byMonth));
});

test("error responses carry a `message` field the client surfaces to the user", async () => {
  const res = await api.post("/api/user").send({ name: "A", email: "bad", address: "x" });
