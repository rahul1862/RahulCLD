...
async function columnsOf(table) {
  const { rows } = await db("SELECT column_name FROM information_schema.columns WHERE table_name = ?", [table]);
  return rows.map((r) => r.column_name);
}

test("users table exists with the expected schema", async () => {
  const columns = await columnsOf("users");
  assert.deepEqual(columns.sort(), ["address", "company", "createdAt", "email", "id", "name", "phone"].sort());
});
...
test("insert then query round-trips a user through the real postgres layer", async () => {
  ...
  await db(
    ...
    [id, "Db Integration", "db.integration@example.com", "Acme Corp", "123 Main Street", "+353 87 000 0000", createdAt]
  );
