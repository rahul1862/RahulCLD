...
test("log helpers accept a message and optional data without throwing", () => {
  assert.doesNotThrow(() => log.info("info message", { a: 1 }));
  assert.doesNotThrow(() => log.warn("warn message"));
  assert.doesNotThrow(() => log.error("error message", { code: "E" }));
...
const fakeReqRes = () => {
  let statusCode;
  let payload;
  const req = { method: "GET", path: "/boom", body: {}, query: {} };
  const res = {
    status(code) {
      statusCode = code;
      return this;
...
test("errorLogger responds with the error's status and message outside production", () => {
  ...
  const err = Object.assign(new Error("something specific broke"), { status: 422 });
