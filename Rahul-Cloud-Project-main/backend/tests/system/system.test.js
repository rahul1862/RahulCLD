...
test("full user lifecycle over real HTTP against the running process", async () => {
  ...
  const get = await fetch(`${BASE_URL}/api/user/${created._id}`, {
    headers: authJson(),
  });
  assert.equal(get.status, 200);
  const list = await fetch(`${BASE_URL}/api/users`, {
    headers: authJson(),
  });
  ...
  const pdf = await fetch(`${BASE_URL}/api/user/${created._id}/pdf`, {
    headers: authJson(),
  });
  ...
  const getAfterDelete = await fetch(`${BASE_URL}/api/user/${created._id}`, {
    headers: authJson(),
  });
