...
export default function Finance() {
  ...
  // transactions and pnl come from separate endpoints, so a mutation needs both refetched
  const refreshAll = () => { reload(); reloadPnl(); };
  ...
  return (
    <div className="page space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-xl divide-y sm:divide-y-0 sm:divide-x divide-ink-100 dark:divide-ink-800 overflow-hidden">
        <Metric label="Income"     value={fmtCurrency(pnl?.totalIncome)} loading={pnlLoading} tone="positive" />
        <Metric label="Expenses"   value={fmtCurrency(pnl?.totalExpense)} loading={pnlLoading} tone="negative" />
              ...
              {loading
                ...
                ? <tr><td colSpan={5}>
                    <EmptyState
                      ...
                      action={<button onClick={() => setAdding(true)} className="btn-primary btn-sm">Add transaction</button>}
                    />
                  </td></tr>
                : filtered.map((t) => (
                  <tr key={t._id}>
                    <td>
                      <span className="text-[13px] font-medium text-ink-800 dark:text-ink-200">{t.category}</span>
                    </td>
                    <td className="hidden sm:table-cell text-[12px] text-ink-500 dark:text-ink-500 truncate max-w-[220px]">
                      {t.description || "—"}
                    </td>
                    <td className="hidden md:table-cell text-[11px] text-ink-400 font-mono whitespace-nowrap">{fmtDate(t.date)}</td>
                    <td className={`text-right text-[13px] font-medium tabular-nums whitespace-nowrap ${t.type === "income" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
