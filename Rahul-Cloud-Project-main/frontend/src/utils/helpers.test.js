...
import { initials, fmtDate, avatarColor, validateUser, fmtCurrency, validateTransaction } from "./helpers";
...
describe("validateUser", () => {
  ...
  test("flags an address shorter than 5 characters", () => {
    const { valid, errors } = validateUser({ name: "Alice Smith", email: "alice@example.com", company: "Acme Corp", address: "Rd" });
    expect(valid).toBe(false);
    expect(errors.address).toBeTruthy();
  });

  test("flags a missing company name", () => {
    const { valid, errors } = validateUser({ name: "Alice Smith", email: "alice@example.com", company: "", address: "123 Main Street" });
    expect(valid).toBe(false);
    expect(errors.company).toBeTruthy();
  });

  test("is valid when every field passes", () => {
    const { valid, errors } = validateUser({ name: "Alice Smith", email: "alice@example.com", company: "Acme Corp", address: "123 Main Street" });
...
describe("validateTransaction", () => {
  test("rejects a type that is not income or expense", () => {
    const { valid, errors } = validateTransaction({ type: "refund", category: "Hosting", amount: 10, date: "2026-01-01" });
    expect(valid).toBe(false);
    expect(errors.type).toBeTruthy();
  });

  test("rejects a category shorter than 2 characters", () => {
    const { errors } = validateTransaction({ type: "income", category: "x", amount: 10, date: "2026-01-01" });
    expect(errors.category).toBeTruthy();
  });

  test("rejects a non-positive or non-numeric amount", () => {
    expect(validateTransaction({ type: "income", category: "Fees", amount: -1, date: "2026-01-01" }).errors.amount).toBeTruthy();
    expect(validateTransaction({ type: "income", category: "Fees", amount: "abc", date: "2026-01-01" }).errors.amount).toBeTruthy();
    expect(validateTransaction({ type: "income", category: "Fees", amount: "", date: "2026-01-01" }).errors.amount).toBeTruthy();
  });

  test("rejects an invalid date", () => {
    const { errors } = validateTransaction({ type: "income", category: "Fees", amount: 10, date: "not-a-date" });
    expect(errors.date).toBeTruthy();
  });

  test("is valid for a well-formed transaction", () => {
    const { valid, errors } = validateTransaction({ type: "expense", category: "Hosting", amount: 25, date: "2026-01-01" });
    expect(valid).toBe(true);
    expect(errors).toEqual({});
  });
});
