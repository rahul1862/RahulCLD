...
http.interceptors.response.use(
  (r) => r,
  (err) => {
    // A 401 means the token is missing/expired — drop it and let the app redirect to login.
...
export const authService = {
  ...
  register: (email, password) => http.post("/auth/register", { email, password }).then((r) => r.data),
