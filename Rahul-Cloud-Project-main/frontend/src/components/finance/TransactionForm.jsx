...
export default function TransactionForm({ initial = {}, onSubmit, onCancel, submitLabel = "Save" }) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      ...
      <div>
        ...
        />
        {errors.category && <p id="category-err" className="mt-1 text-[11px] text-red-500 dark:text-red-400" role="alert">{errors.category}</p>}
      </div>

      <div>
        <label htmlFor="description" className="field-label">
          Description
          <span className="ml-1 text-ink-400 normal-case font-normal tracking-normal">(optional)</span>
        </label>
        <input
          id="description"
          type="text"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="amount" className="field-label">Amount (EUR)</label>
          <input
            id="amount"
            type="number"
            step="0.01"
          ...
          />
          {errors.amount && <p id="amount-err" className="mt-1 text-[11px] text-red-500 dark:text-red-400" role="alert">{errors.amount}</p>}
        </div>
        <div>
          ...
          />
          {errors.date && <p id="date-err" className="mt-1 text-[11px] text-red-500 dark:text-red-400" role="alert">{errors.date}</p>}
        </div>
      </div>
