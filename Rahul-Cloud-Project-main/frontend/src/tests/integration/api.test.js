...
vi.mock("axios", () => ({
  default: { create: () => mockHttp },
}));

const { userService, transactionService, authService, getToken, setToken } = await import("../../services/api");
...
describe("userService integration with axios", () => {
  ...
  test("getAll requests /users and unwraps the response body", async () => {
    mockHttp.get.mockResolvedValue({ data: [{ _id: "1", name: "Alice Smith" }] });
    const result = await userService.getAll();
    expect(mockHttp.get).toHaveBeenCalledWith("/users");
    expect(result).toEqual([{ _id: "1", name: "Alice Smith" }]);
  });

  test("create posts the payload to /user", async () => {
    mockHttp.post.mockResolvedValue({ data: { _id: "2" } });
    const payload = { name: "Alice Smith", email: "alice@example.com", address: "123 Main Street" };
  ...
  test("update puts the payload to /update/user/:id", async () => {
    mockHttp.put.mockResolvedValue({ data: { _id: "3", name: "Updated" } });
    await userService.update("3", { name: "Updated" });
    expect(mockHttp.put).toHaveBeenCalledWith("/update/user/3", { name: "Updated" });
  });

  test("remove issues a DELETE to /delete/user/:id", async () => {
    mockHttp.delete.mockResolvedValue({ data: { message: "Deleted" } });
  ...
  test("search passes the query as a params object", async () => {
    ...
    expect(mockHttp.get).toHaveBeenCalledWith("/search", { params: { q: "alice" } });
  });
  ...
  test("downloadPdf fetches a blob and triggers a client-side download", async () => {
    ...
    mockHttp.get.mockResolvedValue({ data: new Blob(["%PDF-"]) });

    await userService.downloadPdf("5", "alice.pdf");

    expect(mockHttp.get).toHaveBeenCalledWith("/user/5/pdf", { responseType: "blob" });
...
describe("transactionService integration with axios", () => {
  ...
  test("getAll requests /transactions", async () => {
    mockHttp.get.mockResolvedValue({ data: [{ _id: "t1" }] });
  ...
  test("getById requests /transaction/:id", async () => {
    mockHttp.get.mockResolvedValue({ data: { _id: "t2" } });
  ...
  test("create posts to /transaction", async () => {
    mockHttp.post.mockResolvedValue({ data: { _id: "t3" } });
    const payload = { type: "income", category: "Fees", amount: 100, date: "2026-01-01" };
    await transactionService.create(payload);
    expect(mockHttp.post).toHaveBeenCalledWith("/transaction", payload);
  });

  test("update puts to /update/transaction/:id", async () => {
    mockHttp.put.mockResolvedValue({ data: { _id: "t4" } });
    await transactionService.update("t4", { amount: 250 });
    expect(mockHttp.put).toHaveBeenCalledWith("/update/transaction/t4", { amount: 250 });
  });
  ...
  test("pnl requests /pnl", async () => {
    mockHttp.get.mockResolvedValue({ data: { netProfit: 10 } });
    const result = await transactionService.pnl();
    expect(mockHttp.get).toHaveBeenCalledWith("/pnl");
    expect(result).toEqual({ netProfit: 10 });
  });
});

describe("authService", () => {
  test("login posts credentials to /auth/login and unwraps the token payload", async () => {
    mockHttp.post.mockResolvedValue({ data: { token: "jwt-123", user: { email: "a@b.co" } } });
    const result = await authService.login("a@b.co", "secret");
    expect(mockHttp.post).toHaveBeenCalledWith("/auth/login", { email: "a@b.co", password: "secret" });
    expect(result).toEqual({ token: "jwt-123", user: { email: "a@b.co" } });
  });

  test("me requests /auth/me", async () => {
    mockHttp.get.mockResolvedValue({ data: { user: { email: "a@b.co" } } });
    const result = await authService.me();
    expect(mockHttp.get).toHaveBeenCalledWith("/auth/me");
    expect(result).toEqual({ user: { email: "a@b.co" } });
  });

  test("register posts credentials to /auth/register and unwraps the token payload", async () => {
    mockHttp.post.mockResolvedValue({ data: { token: "jwt-456", user: { email: "new@b.co", role: "user" } } });
    const result = await authService.register("new@b.co", "password123");
    expect(mockHttp.post).toHaveBeenCalledWith("/auth/register", { email: "new@b.co", password: "password123" });
    expect(result).toEqual({ token: "jwt-456", user: { email: "new@b.co", role: "user" } });
  });
});
...
describe("response error interceptor", () => {
  ...
  test("maps a server error response to an Error carrying the API message", async () => {
    await expect(
      rejectHandler()({ response: { data: { message: "Email already in use" } } })
  ...
  test("a 401 clears the stored token and emits an auth:unauthorized event", async () => {
    ...
    await expect(rejectHandler()({ response: { status: 401, data: {} } })).rejects.toThrow();
    expect(localStorage.getItem("userhub_token")).toBeNull();
