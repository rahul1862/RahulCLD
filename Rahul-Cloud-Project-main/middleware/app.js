...
// Middleware layer: builds the Express app from injected dependencies
// (the express/cors modules and the backend's db + pdf-library) rather
// than importing them directly, so this folder needs no node_modules
// or package.json of its own — it stays a pure, framework-wired layer
// sitting between the backend service and its data/library layers.
export function createApp({ express, cors, db, generateUserPdf }) {
  ...
  router.post("/user", async (req, res) => {
    ...
    const errors = validate({ name, email, address, company });
    ...
    if (existingRows[0]) return res.status(409).json({ message: "Email already in use" });

    const id = crypto.randomUUID();
    const createdAt = new Date().toISOString();
    await db(
      'INSERT INTO users (id, name, email, company, address, phone, "createdAt") VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, name.trim(), email.trim(), company.trim(), address.trim(), phone?.trim() || null, createdAt]
    );

    res.status(201).json(toApi({ id, name, email, company, address, phone, createdAt }));
  });
  ...
  router.put("/update/user/:id", async (req, res) => {
    ...
    const { rows: dupeRows } = await db("SELECT id FROM users WHERE email = ? AND id != ?", [email, req.params.id]);
    if (dupeRows[0]) return res.status(409).json({ message: "Email already in use" });

    await db("UPDATE users SET name = ?, email = ?, company = ?, address = ?, phone = ? WHERE id = ?", [
      name.trim(),
      email.trim(),
      company.trim(),
      address.trim(),
    ...
    res.json(toApi({ ...row, name, email, company, address, phone }));
  });
  ...
  router.get("/pnl", async (req, res) => {
    ...
    // one pass for both breakdowns, no point walking the rows twice
    const byCategory = {};
    const byMonth = {};
    for (const r of rows) {
      const month = r.date.slice(0, 7); // YYYY-MM
      (byCategory[r.category] ??= { income: 0, expense: 0 })[r.type] += r.amount;
      (byMonth[month] ??= { income: 0, expense: 0 })[r.type] += r.amount;
    }
  ...
  router.post("/transaction", async (req, res) => {
    ...
    const errors = validateTransaction({ type, category, amount, date });
    if (Object.keys(errors).length) {
      return res.status(400).json({ message: Object.values(errors)[0], errors });
    }
    ...
    res.status(201).json(toApiTransaction({ id, type, category, description, amount: Number(amount), date, createdAt }));
  });
  ...
  router.put("/update/transaction/:id", async (req, res) => {
    ...
    await db("UPDATE transactions SET type = ?, category = ?, description = ?, amount = ?, date = ? WHERE id = ?", [
      type,
      category.trim(),
    ...
    res.json(toApiTransaction({ ...row, type, category, description, amount: Number(amount), date }));
  });
